"""
ShuffleNet V2 1.5x Training for Tomato Dataset (Data Augmentation Enabled)
...
def build_shufflenet_v2(input_shape=(224, 224, 3), num_classes=10, model_scale=1.5):
    if model_scale == 0.5:
    ...
    model = tf.keras.Model(inputs=inputs, outputs=outputs, name='ShuffleNetV2_1.5')
    return model
...
def main():
    print("="*60)
    print("ShuffleNet V2 1.5x Training (Data Augmentation Enabled)")
    ...
    print("\nModel oluşturuluyor (ShuffleNet V2 1.5x - sıfırdan, transfer öğrenme yok)...")
    ...
    checkpoint_dir = './checkpoints_tomato_1_5x_aug'
