"""
ShuffleNet V2 2.0x Training for Tomato Dataset (Data Augmentation, GPU)
...
def build_shufflenet_v2(input_shape=(224, 224, 3), num_classes=10, model_scale=2.0):
    if model_scale == 0.5:
    ...
    model = tf.keras.Model(inputs=inputs, outputs=outputs, name='ShuffleNetV2_2.0')
    return model
...
def main():
    print("="*60)
    print("ShuffleNet V2 2.0x Training (Data Augmentation Enabled, GPU)")
    ...
    print("\nModel oluşturuluyor (ShuffleNet V2 2.0x - sıfırdan, transfer öğrenme yok)...")
    ...
    checkpoint_dir = './checkpoints_tomato_2_0x_aug'
